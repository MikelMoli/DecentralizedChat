from tkinter import *
from tkinter import ttk

from uuid import uuid4
from random import randint
from twisted.internet.protocol import DatagramProtocol # Protocol for UDP connections
from twisted.internet import reactor


class Client(DatagramProtocol):

    def __init__(self, host, port, widget):
        # localhost is not accepted on twisted without an IP format
        
        if host == "localhost":
            host = "127.0.0.1"
        
        self.create_chat_window()
        
        self.id = host, port
        self.address = None
        self.server = '127.0.0.1', 9999
        print("Working on id:", self.id)
        # self.id = uuid4() # Sets a random id the client

    def create_chat_window(self):
        print('Entra')
        self.window = Tk()

        # Creating text frame
        
        frame = Frame(self.window, width=600, height=800)
        frame.pack_propagate(False)
        frame.pack()

        # Adds chat input

        input = StringVar()
        input_field = Entry(self.window, text=input)
        input_field.pack(side=BOTTOM, fill=X)

        self.window.mainloop()

    def startProtocol(self):
        self.transport.write("ready".encode("utf-8"), self.server)

    # datagram is the data we receive and address is the address that sends the data
    def datagramReceived(self, datagram: bytes, addr):
        datagram = datagram.decode("utf-8")

        if addr == self.server:
            print("Choose a client from these \n", datagram)
            self.address = input("Write address:"), int(input("Write port:"))
            reactor.callInThread(self.send_message)
        else:
            print(addr, ":", datagram)

    def send_message(self):
        while True:
            self.transport.write(input(":::").encode('utf-8'), self.address)


if __name__ == '__main__':
    port = randint(1000, 5000)
    reactor.listenUDP(port, Client('localhost', port))
    reactor.run()
